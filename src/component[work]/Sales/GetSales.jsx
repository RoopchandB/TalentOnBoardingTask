...
const GetSales = () => {
  ...
  const fetchData = React.useCallback(() => {
    axios
      .get("https://localhost:50001/Sales/GetSalesList")
      .then(({ data }) => {
        setSales(data);
      })
      .catch((err) => {
  ...
  const updateData = React.useCallback(
    (customerName, productName, storeName, proSold) => {
      ...
      if (!saleId) {
        axios
          .post("https://localhost:50001/Sales/AddSales", {
            id: saleId,
        ...
        axios
          .put(`https://localhost:50001/Sales/UpdateSales?Id=${saleId}`, {
            CustomerId: customerName,
            ProductId: productName,
            StoreId: storeName,
            DateSold: proSold,
  ...
  const deleteData = React.useCallback(() => {
    setDeleteOpen((open) => !open);
    axios
      .delete(`https://localhost:50001/Sales/DeleteSales?salesID=${saleId}`)
      .then(() => {
        fetchData();
      })
      .catch((err) => {
